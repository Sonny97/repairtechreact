describe('CP2 - Registro e Inicio de Sesión de Usuario', function () {
  ...
  it('CP2-A - Llenar formulario de registro y validar mensaje de éxito', function () {
    ...
    // ── PASO 20: Validar que aparece el toast de éxito ───────────────────────
    cy.get('.success-toast', { timeout: 8000 }).should('be.visible')
    cy.get('.success-toast').should('contain.text', '¡Registro Exitoso!')
  ...
  it('CP2-C - Login exitoso y redirección a vista de cliente', function () {
    ...
    cy.get('#loginModal', { timeout: 5000 }).should('not.be.visible')

    // ── PASO 9: Validar mensaje de bienvenida en el header ────────────────
    cy.get('.welcome-msg', { timeout: 3000 }).should('be.visible')
    cy.get('.welcome-msg').should('contain.text', 'Hola,')

    // ── PASO 10: Validar que aparece botón "Cerrar sesión" ────────────────
    cy.get('.auth-buttons .btn-secondary')
      .should('contain.text', 'Cerrar sesión')

    // ── PASO 11: Validar que ya no aparece "Registrarse" ─────────────────
    cy.get('.auth-buttons').should('not.contain.text', 'Registrarse')

    // ── PASO 12: Acción — Cerrar sesión ───────────────────────────────────
    cy.get('.auth-buttons .btn-secondary').contains('Cerrar sesión').click()

    // ── PASO 13: Validar que vuelve al estado no autenticado ──────────────
    cy.get('.auth-buttons').should('contain.text', 'Registrarse')
